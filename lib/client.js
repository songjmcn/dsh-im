...
function QrPanel4({ provision, now, busy, onRefresh, onCancel }) {
  const [imageFailed, setImageFailed] = React23.useState(false);
...
function WeixinSettingsTab({ rpcCall }) {
  ...
  const [feedbackByBot, setFeedbackByBot] = React23.useState({});
  const [removeTarget, setRemoveTarget] = React23.useState(null);
  const [notice, setNotice] = React23.useState("");
  const [operationWarnings, setOperationWarnings] = React23.useState([]);
  const [now, setNow] = React23.useState(() => Date.now());
  const addButtonRef = React23.useRef(null);
  const mountedRef = React23.useRef(true);
  const workspaceFence = useWorkspaceSnapshotFence();
  const scheduleAnimationFrame = useAnimationFrameScheduler();
  React23.useEffect(() => {
    mountedRef.current = true;
  ...
  const announce = React23.useCallback((value) => {
    setNotice("");
    scheduleAnimationFrame(() => {
  ...
  const loadStatus = React23.useCallback(async ({
    signal,
    silent = false,
    restoreProvisioning = false
  } = {}) => {
  ...
  const cancelProvisioning = React23.useCallback(async () => {
    setBusy(true);
    try {
      if (provision?.attemptId && !["failed", "expired", "cancelled"].includes(provision.status)) {
        await invoke(WEIXIN_ENDPOINTS.cancelProvisioning, { attemptId: provision.attemptId });
  ...
  const submitVerification = React23.useCallback(async (verifyCode) => {
  ...
  const setBotBusy = React23.useCallback((botId, value) => {
    setBusyByBot((current) => {
      const next = { ...current };
      if (value) next[botId] = value;
      else delete next[botId];
  ...
  const reconnect = React23.useCallback(async (account) => {
    const snapshotVersion = workspaceFence.beginMutation();
  ...
  const saveWorkspace = React23.useCallback(async (account, workspace) => {
    const workspaceVersion = workspaceFence.beginMutation();
    setBotBusy(account.botId, "workspace");
  ...
  const saveBotSetting = React23.useCallback(async (account, operation, endpoint, payload) => {
    const snapshotVersion = workspaceFence.beginMutation();
    setBotBusy(account.botId, operation);
    setFeedbackByBot((current) => ({ ...current, [account.botId]: null }));
    try {
  ...
  const remove = React23.useCallback(async (account) => {
    const snapshotVersion = workspaceFence.beginMutation();
...
function WhatsappSettingsTab({ rpcCall }) {
  ...
  const [testNoticeByBot, setTestNoticeByBot] = React24.useState({});
  ...
  const loadStatus = React24.useCallback(async ({ signal, silent = false, restore = false } = {}) => {
    const workspaceVersion = workspaceFence.beginStatus();
    if (workspaceVersion === null) return void 0;
    if (!silent && mounted.current) setModel((current) => ({ ...current, phase: "loading", error: null }));
    try {
  ...
  const closeProvision = React24.useCallback(async () => {
    setBusy(true);
    try {
  ...
  const botAction = React24.useCallback(async (account, operation, endpoint, payload) => {
    const snapshotVersion = workspaceFence.beginMutation();
    setBusyByBot((current) => ({ ...current, [account.botId]: operation }));
    if (operation === "reconnect") {
      setTestNoticeByBot((current) => {
...
function ScenePolicyEditor({
  ...
  const ownerHelpId = React26.useId();
  const emptyAllowlistHelpId = React26.useId();
...
function AccessPolicySettingsPage({ channel: channel5, account, rpcCall, onSaved }) {
  ...
  const [draft, setDraft] = React26.useState(() => clonePolicy(
    initialPolicy ?? DEFAULT_ACCESS_POLICY
  ));
  const [saving, setSaving] = React26.useState(false);
  const [feedback, setFeedback] = React26.useState(null);
  React26.useEffect(() => {
...
function GroupResponseModeEditor({
  ...
  const [authorizing, setAuthorizing] = React27.useState(false);
...
function FeishuGroupSettingsPage({ account, rpcCall }) {
  const [settings, setSettings] = React27.useState(() => groupSettingsFrom(account));
  const [refreshing, setRefreshing] = React27.useState(false);
  const [refreshError, setRefreshError] = React27.useState(null);
  const [provision, setProvision] = React27.useState(null);
  const [provisionBusy, setProvisionBusy] = React27.useState(false);
  const [now, setNow] = React27.useState(() => Date.now());
  ...
  const applySnapshot = React27.useCallback((value) => {
    const result = targetBotFromSnapshot(value, account.botId);
    if (mounted.current) setSettings(groupSettingsFrom(result.bot));
    return result;
  }, [account.botId]);
  const loadSettings = React27.useCallback(async ({ signal, restoreProvisioning = false } = {}) => {
    setRefreshing(true);
  ...
  const saveSetting = React27.useCallback(async (endpoint, payload) => {
    const value = await invoke(endpoint, { botId: account.botId, ...payload });
    applySnapshot(value);
  }, [account.botId, applySnapshot, invoke]);
  const startAuthorization = React27.useCallback(async ({ replace = false } = {}) => {
  ...
  const finishAuthorization = React27.useCallback(async (signal) => {
  ...
  const cancelAuthorization = React27.useCallback(async () => {
...
function TargetForm({
  ...
  const [targetId, setTargetId] = React28.useState(initialValue?.targetId ?? "");
  const [name2, setName] = React28.useState(initialValue?.name ?? "");
  const [kind, setKind] = React28.useState(initialKind);
  const [route, setRoute] = React28.useState(initialValue?.route ?? {});
  const [error, setError] = React28.useState(null);
  const [testing, setTesting] = React28.useState(false);
  const [testState, setTestState] = React28.useState(null);
  const currentTarget = () => {
    ...
    phase === "loading" ? h2("div", { className: "dim-targetSuggestionState", "aria-busy": "true" }, "\u6B63\u5728\u8BFB\u53D6\u5DF2\u804A\u4F1A\u8BDD\u2026") : phase === "error" ? h2(
      ...
      h2(
        ...
        suggestions.map((suggestion, index) => {
          ...
          return React28.createElement("option", {
...
function TargetRow({ definition, target, botId, connected, rpcCall, onChanged, onEdit }) {
  const [action, setAction] = React28.useState(null);
  const [testState, setTestState] = React28.useState(null);
  const [syncFeedback, setSyncFeedback] = React28.useState(null);
  const [copyState, setCopyState] = React28.useState(null);
  const [confirmDelete, setConfirmDelete] = React28.useState(false);
...
function DeliveryTargetSettingsPage({
  ...
  const [activeTabId, setActiveTabId] = React28.useState(BOT_SETTINGS_TABS[0].id);
  const [phase, setPhase] = React28.useState("loading");
  const [targets, setTargets] = React28.useState([]);
  const [suggestionPhase, setSuggestionPhase] = React28.useState("idle");
  const [suggestions, setSuggestions] = React28.useState([]);
  const [suggestionError, setSuggestionError] = React28.useState(null);
  const [error, setError] = React28.useState(null);
  const [editor, setEditor] = React28.useState(null);
  const [saving, setSaving] = React28.useState(false);
  const [botCopyState, setBotCopyState] = React28.useState(null);
  const [accessPolicy, setAccessPolicy] = React28.useState(account.accessPolicy);
  ...
  const loadTargets = React28.useCallback(async ({ signal, silent = false } = {}) => {
  ...
  const loadSuggestions = React28.useCallback(async () => {
...
function GlobalSettingsPanel({ rpcCall }) {
  ...
  const [loadError, setLoadError] = React29.useState(null);
  const [ttlInput, setTtlInput] = React29.useState("");
  const [savedTtl, setSavedTtl] = React29.useState(null);
  const [ttlError, setTtlError] = React29.useState(false);
  const [saveError, setSaveError] = React29.useState(null);
  const [saveSucceeded, setSaveSucceeded] = React29.useState(false);
  const [isSaving, setIsSaving] = React29.useState(false);
  const [sweepConfirming, setSweepConfirming] = React29.useState(false);
  const [sweeping, setSweeping] = React29.useState(false);
  const ttlErrorId = React29.useId();
  const ttlHintsId = React29.useId();
  const sweepTriggerId = React29.useId();
  const sweepConfirmId = React29.useId();
  const sweepConfirmTextId = React29.useId();
  const sweepConfirmButtonId = React29.useId();
  const mounted = React29.useRef(true);
  const saving = React29.useRef(false);
  ...
  const loadSettings = React29.useCallback(async ({ signal } = {}) => {
    setPhase("loading");
    setLoadError(null);
    try {
...
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  const [copyState, setCopyState] = React30.useState("idle");
  const commandRef = React30.useRef(null);
  const mounted = React30.useRef(false);
  const copying = React30.useRef(false);
  React30.useEffect(() => {
    mounted.current = true;
    return () => {
      mounted.current = false;
...
function UpdateDialog({ children, onClose }) {
  const dialogRef = React30.useRef(null);
  const titleId = React30.useId();
  const descriptionId = React30.useId();
  React30.useEffect(() => {
    const previous = globalThis.document?.activeElement;
    dialogRef.current?.focus?.();
    return () => {
...
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  const [snapshot, setSnapshot] = React30.useState(null);
  const [action, setAction] = React30.useState("status");
  const [error, setError] = React30.useState(null);
  const [open, setOpen] = React30.useState(false);
  const [uncertainInstall, setUncertainInstall] = React30.useState(false);
  const mounted = React30.useRef(false);
  const busy = React30.useRef(false);
  const readController = React30.useRef(null);
  const pollReadController = React30.useRef(null);
  const installRequest = React30.useRef(null);
  const onStatusRef = React30.useRef(onStatus);
  onStatusRef.current = onStatus;
  const accept = React30.useCallback((next) => {
    setSnapshot(next);
